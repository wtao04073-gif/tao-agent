...
**M0 验证后确认「预计要改的三处全部不需要改」，源码零改动。** 现有改动仅 1 处，且是工程配置而非源码逻辑。

vendor 与上游 v0.87.1 的源码逐字节一致（差异仅：补入上游 gitignore 掉的 `ai/src/providers/data/`，以及下表登记的配置文件）。没有源码补丁意味着季度升级基本是覆盖即可。

## 改动登记

| # | 文件 | 改动摘要 | 原因 | 能否用扩展点替代 | 日期 |
|---|---|---|---|---|---|
| P001 | `telemetry/vitest.config.ts`、`chord/vitest.config.ts` | 新增最小 vitest 配置 | 隔离仓库根配置的意外继承，不改变上游测试行为 | 否（配置查找是工具行为） | 2026-09-24 |

### P001 · 为 telemetry 与 chord 补 vitest 配置

- **文件**：`vendor/pi/telemetry/vitest.config.ts`、`vendor/pi/chord/vitest.config.ts`（新增）
- **上游对应**：这两个包在上游**没有**该文件
- **改动类型**：新增（不修改任何上游文件）
- **原因**：这两个包原本靠 vitest 默认行为发现 `test/`。本仓库根目录有 `vitest.config.ts`（自有包与 e2e 用，`include` 指向 `packages/*/test/`），vitest 会向上查找并继承它，导致这两个包报 `No test files found`。**这是我们引入的环境副作用，不是上游缺陷。**
- **改动内容**：各新增一份最小配置，仅声明 `include: ["test/**/*.test.ts"]` 与 node 环境，等价于上游的默认行为
- **影响面**：仅影响测试发现，不触碰任何生产代码
- **测试覆盖**：`npm run test:vendor` —— telemetry 15 项、chord 282 项恢复通过
- **升级注意**：若上游将来自带了 `vitest.config.ts`，**直接采用上游版本并删除本补丁**；只需确认其 `include` 能覆盖 `test/`

> 💡 这两个文件带 `// [TAO-PATCH]` 标记，可用 `grep -rn "TAO-PATCH" vendor/` 列全。

## 登记格式

新增改动时按上表追加，并在正文补充详细说明：
